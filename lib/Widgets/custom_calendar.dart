...
class CustomDeliveryCalendar extends StatefulWidget {
  final Function finish;
  const CustomDeliveryCalendar({required this.finish, Key? key}) : super(key: key);

  @override
  _CustomDeliveryCalendarState createState() => _CustomDeliveryCalendarState();
}

class _CustomDeliveryCalendarState extends State<CustomDeliveryCalendar> {
  PageController control = PageController();
  Timeslot? date;
  int slot = 0;
          ...
          child: GridView.builder(
            ...
            itemBuilder: (context, index) {
              return CustomTextButton(
                ...
                colourUnpressed: (date!.deliveries![index] == "") ? null : app.mResource.colours.otherMonth,
                active: (date!.deliveries![index] == ""),
              );
            },
          ),
        ),
    ...
    return GridView.builder(
      ...
      itemBuilder: (BuildContext context, int index) {
        ...
        return CustomTextButton(
          ...
          colourUnpressed: (app.mData.calendar!.current!.days[index - dayOneIndex].available > 0) ? app.mResource.colours.activeDateUnpressed : app.mResource.colours.inactiveDate,
          colourPressed: app.mResource.colours.activeDatePressed,
          active: (app.mData.calendar!.current!.days[index - dayOneIndex].available > 0) && (app.mData.calendar!.current!.days[index - dayOneIndex].deliverCount! < 2),
        );
      },
    );
  }
